...
class Cal_Con(QWidget, Ui_Form):
    ...
    def on_Show_Laser_clicked(self):
        ...
        try:
            ...
            if self.index == 0:
                ...
                self.x_laser_lim = np.arange(
                    (-self.laser_width), self.laser_width, step=0.1)
                self.num_laser = len(self.x_laser_lim)
                self.laser = self.gate_function(
                    self.x_laser_lim, self.laser_width / 2)
                fig = Figure()
                self.ax_laser = fig.add_subplot(111)
                self.ax_laser.set_xlim([(-self.laser_width), self.laser_width])
                self.ax_laser.set_xlim([0, 1])
                self.ax_laser.cla()  # 清除坐标轴
                self.ax_laser.plot(self.x_laser_lim, self.laser)
                ...
                self.miu = float(self.Set_Miu.text())
                self.gauss_im_width = float(self.Set_Gas_Width.text())
                self.laser_width = float(self.Set_Sigma.text())
                self.x_laser_lim = np.arange(
                    (-self.gauss_im_width), self.gauss_im_width, step=0.1)
                self.num_laser = len(self.x_laser_lim)
                self.laser_no = self.gaussian(
                    self.x_laser_lim, self.miu, self.laser_width)
    ...
    def on_Save_Laser_clicked(self):
        ...
        try:
            file_path = QFileDialog.getSaveFileName(
                self, 'save file', 'C:\\Users\\Sai\\Desktop', 'image files(*.png *.svg )')[0]
    ...
    def on_Show_Res_clicked(self):
        ...
        try:
            ...
            con_result = signal.fftconvolve(self.laser, self.gate, mode='full')
